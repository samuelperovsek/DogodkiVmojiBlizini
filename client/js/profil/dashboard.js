...
import { potrdiAkcijo, pobegniHtml, varnoUrl } from '../components.js';
      ...
      <div class="upcoming-event">
        <div class="d-flex align-items-center gap-3">
          ...
          <div class="flex-grow-1">
            <span class="badge bg-light text-dark">${pobegniHtml(p.kategorija)}</span>
            <small class="text-muted ms-2"><i class="bi bi-clock"></i> ${ura} • <i class="bi bi-geo-alt"></i> ${pobegniHtml(p.kraj)}</small>
            <h5 class="mb-0 mt-1" style="font-size: 0.98rem;"><a href="dogodek.html?id=${Number(p.id)}">${pobegniHtml(p.naslov)}</a></h5>
            ${opomnik}
          </div>
    ...
    return `
      <div class="d-flex gap-3 mb-3 align-items-center">
        ...
        <div class="flex-grow-1" style="min-width: 0;">
          <small class="text-muted d-block"><i class="bi bi-geo-alt"></i> ${pobegniHtml(d.kraj)}</small>
          <strong class="d-block" style="font-size:0.92rem;"><a href="dogodek.html?id=${Number(d.id)}">${pobegniHtml(d.naslov)}</a></strong>
        </div>
      </div>
    `;
  }).join('');
    ...
    return `
      <div class="col-md-6">
        <div class="card p-3 h-100 position-relative">
          ...
          <h5 class="mb-1" style="font-size:1rem; padding-right: 1.5rem;"><a href="dogodek.html?id=${Number(o.dogodek_id)}">${pobegniHtml(o.dogodek_naslov)}</a></h5>
          <div class="text-warning mb-2" style="font-size:0.9rem;">
            ${zvezde}
            <small class="text-muted ms-2">— ${datum}</small>
          </div>
          <p class="mb-0" style="font-size:0.9rem;">${o.komentar ? pobegniHtml(o.komentar) : '<em class="text-muted">Brez komentarja</em>'}</p>
        </div>
      </div>
    `;
  }).join('') + '</div>';
